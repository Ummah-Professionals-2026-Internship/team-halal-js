...
const PageLayoutDashboard = ({ children, userName, userRole, userPhoto, onPhotoUpdate, onBack }) => {
  ...
  return (
    <div className="h-screen flex flex-col bg-[#f6f6f6] overflow-hidden">
      ...
      <div className="shrink-0 flex justify-between items-center px-6 py-3 border-t border-slate-200 bg-[#f6f6f6]">
        {onBack
          ? <button onClick={onBack} className="bg-[#003F55] text-white px-6 py-2 rounded font-bold shadow">Back</button>
          : <div />
        }

        <div className="flex gap-3 text-xs text-slate-400 font-semibold items-center">
          <Link to="/privacy-policy" className="hover:underline hover:text-[#003F55] transition-colors">Privacy Policy</Link>
          <span>•</span>
          <Link to="/terms-of-use" className="hover:underline hover:text-[#003F55] transition-colors">Terms of Use</Link>
        </div>
