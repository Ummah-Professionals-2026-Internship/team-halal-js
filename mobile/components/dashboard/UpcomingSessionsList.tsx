  ...
  return (
    <View className="bg-white rounded-2xl p-5 border border-brand-cardBorder" style={cardShadow}>
      <View className="flex-row items-center justify-between mb-4">
        <Text className="text-base text-brand-text" style={fontBoldStyle}>Upcoming Sessions</Text>
        <View className="bg-brand-accent/15 rounded-full px-3 py-1">
          <Text className="text-xs text-brand-text" style={fontBoldStyle}>{upcoming.length}</Text>
        </View>
      </View>

      {upcoming.length === 0 ? (
        <Text className="text-xs text-slate-500 mb-2" style={fontStyle}>No upcoming sessions scheduled yet.</Text>
      ) : (
      ...
      {completed.length > 0 && (
        <>
          <Text className="text-base text-brand-text mt-4 mb-4" style={fontBoldStyle}>
